...
class ProfessorService
{
    ...
    public function listarValidacoesPendentes(int $professorId, ?int $turmaId = null): array {
        try {
            $sql = "SELECT se.id, u.nome_completo AS aluno_nome, se.etapa_id AS numero_etapa, 
                           se.data_submissao AS data, se.titulo_submissao AS titulo, se.descricao_submissao AS texto,
                           t.codigo_turma
                    FROM submissoes_etapa se
                    INNER JOIN usuarios u ON se.aluno_id = u.id
                    INNER JOIN turmas t ON u.turma_id = t.id 
            ...
            // Em ambiente de produção, seria ideal logar esse erro: error_log($e->getMessage());
            return [];
        }
    }
    ...
    public function obterProgressoAluno(int $alunoId): array {
        try {
            // 1. Busca a última etapa que o aluno foi APROVADO
            $stmtEtapa = $this->conexao->prepare("
                SELECT MAX(etapa_id) FROM submissoes_etapa 
                WHERE aluno_id = :id AND status LIKE 'APROVADO%'
            ");
            $stmtEtapa->bindValue(':id', $alunoId, PDO::PARAM_INT);
            $stmtEtapa->execute();
            $ultimaEtapaAprovada = (int) $stmtEtapa->fetchColumn();

            // 2. Busca o total de etapas cadastradas no sistema
            $stmtTotal = $this->conexao->query("SELECT COUNT(*) FROM etapas");
            $totalEtapas = (int) $stmtTotal->fetchColumn();
            
            if ($totalEtapas === 0) {
                $totalEtapas = 5; // Fallback de segurança se a tabela de etapas estiver vazia
            }

            // Se o aluno ainda não aprovou nenhuma etapa, está zerado
            ...
            // 3. Calcula a percentagem do ciclo concluída
            $percentual = ($ultimaEtapaAprovada / $totalEtapas) * 100;
            if ($percentual > 100) $percentual = 100;

            return [
                'nivel' => $ultimaEtapaAprovada, // Usamos a etapa atual como "nível" de progresso
            ...
            return ['nivel' => null, 'percentual_progresso' => 0, 'etapa_atual' => 0, 'total_etapas' => 0];
        }
    }
    ...
    {
        try {
            $sql = "SELECT ck.id AS conceito_id, ck.termo
                    ...
                    GROUP BY ck.id, ck.termo, ck.ordem
                    ORDER BY ck.ordem ASC";
            $stmt = $this->conexao->prepare($sql);
            $stmt->bindValue(':sid', $submissaoId, PDO::PARAM_INT);
            $stmt->execute();
            $conceitos = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $stmt->closeCursor();

            $resultado = [];
            foreach ($conceitos as $c) {
                $conceitoId = (int) $c['conceito_id'];
                $stmtF = $this->conexao->prepare(
                    "SELECT f.nome, f.epoca, f.linha_pensamento, f.ideias_principais, f.citacao
                     FROM filosofos_etapa4 f
                     INNER JOIN conceito_filosofo cf ON cf.filosofo_id = f.id
                     WHERE cf.conceito_chave_id = :cid AND f.submissao_etapa_id = :sid
                     ORDER BY f.id ASC"
                );
                $stmtF->execute([':cid' => $conceitoId, ':sid' => $submissaoId]);
                $filosofos = [];
                foreach ($stmtF->fetchAll(PDO::FETCH_ASSOC) as $f) {
                    $epoca = (string) $f['epoca'];
                    $filosofos[] = [
                        'nome' => (string) $f['nome'],
                        'epoca' => $epoca,
                        'epoca_rotulo' => \PhiloQuest\Etapa4FilosofoHelper::rotuloEpoca($epoca),
                        'linha_pensamento' => (string) $f['linha_pensamento'],
                        'ideias_principais' => (string) $f['ideias_principais'],
                        'citacao' => (string) $f['citacao'],
                    ];
                }
                $stmtF->closeCursor();
    ...
    public function avaliarSubmissao(
        ...
        try {
            ...
            // Se ganhou XP (ou seja, foi aprovado), salva no histórico e na conta do aluno
            if ($xpGanho > 0) {
                // Insere no histórico de XP
                ...
                // Atualiza o total de XP no perfil do utilizador
            ...
            throw new Exception("Erro ao avaliar: " . $e->getMessage());
        }
    }
}