...
function obterCoresStatus($status) {
    return match($status) {
        'APROVADO', 'APROVADO_BEM_FEITO', 'APROVADO_EXCELENTE' => ['cor' => '#27AE60', 'bg' => '#E8F5E9'],
        'NECESSITA_REVISAO' => ['cor' => '#E53E3E', 'bg' => '#FEE2E2'],
        default => ['cor' => '#718096', 'bg' => '#EDF2F7'],
    };
}

function formatarNomeStatus($status) {
    return match($status) {
        'APROVADO' => 'Aprovado',
        'APROVADO_BEM_FEITO' => 'Aprovado - Bem Feito',
        'APROVADO_EXCELENTE' => 'Aprovado - Excelente',
...
<?php else: ?>
    <?php foreach ($avaliacoes as $av):
        ...
        $cores = obterCoresStatus($av['status']);
        $nomeStatus = formatarNomeStatus($av['status']);
        $dataExibicao = date('d/m/Y H:i', strtotime($av['data_validacao']));
    ?>
    <?php
            ...
            <?php if ($av['status'] === 'NECESSITA_REVISAO'): ?>
                <a href="ciclo.php" class="badge rounded-pill text-decoration-none"
                   style="background-color: <?= htmlspecialchars($cores['bg']) ?>; color: <?= htmlspecialchars($cores['cor']) ?>;">
                    <?= $nomeStatus ?> <i class="fas fa-edit ms-1"></i>
                </a>
            <?php else: ?>
        ...
        <div class="card-body px-4 pb-4">
            <div class="row g-4">
                <div class="col-md-3">
                    ...
                    <?php if ($numeroEtapaAv === 5 && $av['status'] !== 'NECESSITA_REVISAO' && $av['nota'] !== null && $av['nota'] !== ''): ?>
                        <?php
                            $notaValor = (float) $av['nota'];
                            $notaExibicao = fmod($notaValor, 1.0) === 0.0
                                ? (string) (int) $notaValor
                                : number_format($notaValor, 1, ',', '.');
                        ?>
                        <div class="fw-bold fs-5 text-primary mt-2 mb-2">
                            <i class="fas fa-graduation-cap"></i> Nota: <?= htmlspecialchars($notaExibicao) ?>/10
                        </div>
                    <?php endif; ?>
                    <p class="small fw-semibold text-muted mb-1">Avaliador</p>
                    <p class="mb-0"><i class="fas fa-chalkboard-teacher text-primary"></i> Prof. <?= htmlspecialchars($av['professor_nome']) ?></p>
                </div>
                <div class="col-md-9">
                    <h4 class="h6 fw-semibold text-dark">Parecer do Professor</h4>
